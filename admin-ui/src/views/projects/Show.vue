<template>
  <div>
    ...
    <div>
      <div>
        <v-text-field label="機能名" v-model="featureForm.name"></v-text-field>
      </div>

      <div>
        <label>キー(半角英数)</label>
        <input v-model="featureForm.key" placeholder="キー">
      </div>

      <v-btn @click="postFeature">作成</v-btn>
    </div>
    ...
    <ul v-if="users.length > 0">
      <li v-for="user in users" :key="user.uuid">
        <p>
          ...
          <router-link :to="`/projects/${project._id}/users/${user.uuid}`">詳細</router-link>
        </p>
      </li>
    </ul>
    <p v-else>ユーザが登録されていません</p>
  </div>
</template>
